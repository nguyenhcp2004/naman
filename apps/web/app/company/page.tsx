...
export default async function CompanyPage() {
  const page = await sanityFetch<StaticPage>({ query: STATIC_PAGE_QUERY, params: { pageType: "about" } })
  ...
  return (
    <section className="mx-auto flex min-h-[calc(100svh-9rem)] w-full max-w-7xl flex-col justify-center px-5 py-16 sm:px-8">
      ...
      <h1 className="max-w-3xl text-5xl font-semibold tracking-[-0.05em] text-foreground md:text-7xl">
        {page?.section?.title ?? page?.title ?? "Meet the company behind QMaster."}
      </h1>
      <p className="mt-6 max-w-2xl text-lg leading-8 text-muted-foreground">
        {page?.section?.subtitle ?? "Learn how our team helps organizations turn operational noise into confident execution."}
      </p>
    </section>
  )
}
